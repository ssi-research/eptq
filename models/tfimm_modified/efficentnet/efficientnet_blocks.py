...
class MDepthwiseSeparableConv(object):
    def __init__(self, cfg: BlockArgs, name, **kwargs):
        self.cfg = cfg
        # This depends on number of input channels and is set during build phase
        self.skip_connection = None
        ...
        self.bn1 = norm_layer(name=f"{name}.bn1")
        self.act1 = act_layer()
        self.se = (
            SqueezeExcite(rd_ratio=cfg.se_ratio, act_layer=cfg.act_layer, name=f"{name}.se")
...
class MInvertedResidual(object):
    def __init__(self, cfg: BlockArgs, name, **kwargs):
        ############################################
        # Build Layers
        ############################################
        ...
        self.conv_pwl = create_conv2d(
            ...
            kernel_size=cfg.pw_kernel_size,
            padding=cfg.padding,
            name=f"{name}.conv_pwl",
        )
        self.bn3 = norm_layer(name=f"{name}.bn3")

    def __call__(self, x: tf.Tensor):
        in_channels = x.shape[-1]
        if self.conv_pw is None:
            self.conv_pw = create_conv2d(
